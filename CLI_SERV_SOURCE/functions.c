	...
	char *argv[];
{
	...
	if (argc != 4) {
		fprintf(stderr, "usage: %s [ip] [service_1/service_2/service_3] [message]\n", argv[0]);
		exit(1);
	}
	...
	if(strcmp(service, "service_1") == 0) {
		clnt = clnt_create(server, SERVICE_1, PRINTMSG_1, "tcp");
		if (clnt == (CLIENT *)NULL) {
		...
		clnt = clnt_create(server, SERVICE_2, PRINTMSG_2, "tcp");
		if (clnt == (CLIENT *)NULL) {
			syslog(LOG_INFO, "could't establish connection with server %s, aborting", server);
			clnt_pcreateerror(server);
			exit(1);
		...
		clnt = clnt_create(server, SERVICE_3, PRINTMSG_3, "tcp");
		if (clnt == (CLIENT *)NULL) {
			syslog(LOG_INFO, "could't establish connection with server %s, aborting", server);
			clnt_pcreateerror(server);
			exit(1);
